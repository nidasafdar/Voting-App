...
router.post('/', async (req, res) =>{
    try{
        ...
        if (!/^\d{12}$/.test(data.CNIC)) {
            return res.status(400).json({ error: 'CNIC Number must be exactly 12 digits' });
        }
...
router.post('/login', async(req, res) => {
    try{
        // Extract CNIC and password from request body
        const {CNIC, password} = req.body;

        // Check if CNIC or password is missing
        if (!CNIC || !password) {
        ...
        if( !user || !(await user.comparePassword(password))){
            return res.status(401).json({error: 'Invalid Aadhar CNIC or Password'});
        }
